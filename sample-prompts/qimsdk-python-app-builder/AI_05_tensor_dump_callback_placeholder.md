Create a QIM SDK Python app that runs inference and exposes tensor output through a custom tensor callback placeholder.

## Pipeline Behavior
- Decode an MP4 file using Qualcomm hardware decoder
- Run TFLite inference using external delegate on HTP/NPU
- Use custom Python postprocess with `Tensors` output marker
