...
/*
 * Application: Programmatic GStreamer Security Camera Usecase
 *
 * Description:
 *   This application demonstrates a real-time object detection pipeline
 *   using GStreamer and a YOLOv8 TensorFlow Lite model. It processes video
 *   streams from various sources, detects objects and attaches metadata,
 *   visualizes results with bounding boxes, and streams the annotated
 *   output over RTSP or WebRTC for real-time monitoring.
 *
 ...
 *
 * Example usage:
 *   ./gst-ip-camera --input-type=isp --width=1280 --height=720
 *
 *   ./gst-ip-camera \
 *       --input-type=usb \
    ...
    GstElement * output_head, GstElement * meta_head)
{
  ...
  GstElement *postprocess_caps = NULL, *qtivoverlay = NULL, *qtimlmetaparser = NULL;
  ...
  gst_bin_add_many (GST_BIN (appctx->pipeline),
      ...
      qtimetamux, qtivoverlay, qtimlmetaparser, NULL);
  ...
  gst_element_link_many (qtimetamux, queue[5], qtivoverlay, tee[1], NULL);
...
main (int argc, char * argv[])
{
  ...
  option_ctx = g_option_context_new ("- Runs a gstreamer object detection "
