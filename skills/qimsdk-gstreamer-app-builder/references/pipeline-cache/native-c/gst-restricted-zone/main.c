...
/*
 * Application: Programmatic GStreamer Restricted Zone Usecase
 *
 * Description:
 *   This application demonstrates a real-time restricted zone monitoring
 *   pipeline using GStreamer. It processes video streams to detect people’s
 *   feet, evaluates their position relative to a predefined area, and
 *   highlights violations directly in the video. The annotated video and
 *   metadata can be streamed over RTSP or WebRTC.
 *
 ...
 *
 * Example usage:
 *   ./gst-restricted-zone --input-type=isp --width=1280 --height=720
 *
 *   ./gst-restricted-zone \
 *       --input-type=usb \
...
#define MODEL_PATH "foot_track_net_quantized.tflite"
#define LABELS_PATH "foot_track_net.json"
#define SETTINGS_PATH "foot_track_net_settings.json"
    ...
    GstElement * output_head, GstElement * meta_head)
{
  ...
  GstElement *qtimlpostprocess = NULL, *qtimetamux = NULL;
  GstElement *postprocess_caps = NULL, *qtivoverlay = NULL, *qtiobjtracker = NULL;
  GstElement *qtimlmetaparser = NULL, *qtirestrictedzonedbg = NULL;
  GstElement *tee[TEE_COUNT], *queue[QUEUE_COUNT];
  GstCaps *pad_filter = NULL;
  gchar element_name[128];

  gchar *overlay_rz =
      g_strdup ("{(structure)\"Zone1,polygon=<<640,420>,<687,408>,<843,476>,"
                "<848,528>,<937,583>,<641,729>>,color=0x7F00007F;\"}");
  ...
  qtiobjtracker =
    gst_app_make_element ("qtiobjtracker", "qtiobjtracker");

  qtirestrictedzonedbg =
    gst_app_make_element ("qtirestrictedzonedbg", "qtirestrictedzonedbg");
  ...
  if (!qtimlvconverter  || !qtimltflite   || !qtimlpostprocess ||
      !postprocess_caps || !qtimetamux    || !qtivoverlay      ||
      !qtimlmetaparser  || !qtiobjtracker || !qtirestrictedzonedbg) {
  ...
  gchar *settings_path = g_strdup_printf("%s/labels/%s",
      appctx->config.model_base_path, SETTINGS_PATH);
  ...
  g_object_set (G_OBJECT (postprocess_caps), "caps", pad_filter, NULL);
  ...
  gst_bin_add_many (GST_BIN (appctx->pipeline),
      qtimlvconverter, qtimltflite, qtimlpostprocess, postprocess_caps,
      qtimetamux, qtivoverlay, qtiobjtracker, qtimlmetaparser,
      qtirestrictedzonedbg, NULL);
  ...
  // Linking input to tee 1. Then linking tee 1 to metamux
  gst_element_link_many (input_tail, tee[0], queue[0], qtimetamux, NULL);

  // Linking input to ML processing -> metamux
  ...
  // Linking ML text output to meta_head (JSON meta stream)
  if (meta_head != NULL)
...
gst_app_create_pipe (GstAppContext * appctx)
{
  ...
  appctx->pipeline = gst_pipeline_new ("gst-restricted-zone");
...
main (int argc, char * argv[])
{
  ...
  option_ctx = g_option_context_new ("- Runs a gstreamer restricted zone "
