#!/usr/bin/env python3
"""BM25 retrieval over the qimsdk-gstreamer-app-builder pipeline example indexes.
...
the two example corpora shipped alongside this skill:
    ...
    skills/qimsdk-gstreamer-app-builder/references/pipeline-index.gst-launch.jsonl  (gst-launch/*.sh one-liners)
    skills/qimsdk-gstreamer-app-builder/references/pipeline-index.native-c.jsonl    (native-c/<app-dir> C apps)

Each line in those files is a JSON object: {"prompt": "<nl description>",
"path": "gst-launch/<slug>.sh"} or {"prompt": "...", "path": "native-c/<dir>"}.
...
The two .jsonl files are tokenized once and cached to
    skills/qimsdk-gstreamer-app-builder/references/pipeline-cache/.index-cache.json
...
CLI
    python rank_examples.py --query "<text>" [--pool {gst-launch,native-c,both}]
                             [--top-k N] [--task TAG]
...
EXAMPLES
    ...
    python rank_examples.py --query "gesture recognition" --pool native-c
    ...
    python rank_examples.py --query "detect objects from usb-camera" --task object-detection --pool gst-launch
...
INDEX_FILES = {
    ...
    "native-c": CORPUS_DIR / "pipeline-index.native-c.jsonl",
}
POOL_NAMES = ("gst-launch", "native-c")  # fixed, deterministic iteration order
...
SYNONYMS = {
    ...
    "hrnet": ["pose"],
    "keypoint": ["pose"],
    # multi-model chaining -- corpus is split between "daisychain" (native-c)
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="BM25 retrieval over the qimsdk-gstreamer-app-builder pipeline example indexes.",
    ...
    parser.add_argument(
        "--pool",
        choices=["gst-launch", "native-c", "both"],
        default="both",
