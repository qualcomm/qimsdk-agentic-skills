...
## C++ SDK-Specific Additions

The inventory above is the complete shared QIM GStreamer plugin fact catalog.
The following SDK-specific names are retained from the C++ registry references:

- `camsrc` — SDK registry alias for the ISP camera source. Prefer the user-requested source name and the documented `qticamsrc`/`qtiqmmfsrc` behavior when generating new code.
- `qtimlvideotflitebin`, `qtimlvideoqnnbin`, `qtimlvideosnpebin`, and `qtimlvideoonnxbin` — ML-bin wrapper names; use the ML-bin property family documented in the shared inventory and the C++ construction rules.

SDK wrapper methods such as `set_handler(...)`, `set_preprocess_handler(...)`,
and `set_postprocess_handler(...)` are API facts, not plugin properties. Keep
their detailed construction guidance in `api-surface.md`,
`pipeline-construction.md`, and `ml-and-postprocess.md`.

## C++ Generation Defaults Retained
...
- For decoded file/RTSP H.264 input, use `v4l2h264dec capture-io-mode=4 output-io-mode=4`.
- Display defaults to `waylandsink fullscreen=true sync=true` for every source type, including live camera sources — camera source type alone is not a reason to use `sync=false`. See "Display Sink Sync Policy" in `pipeline-construction.md` for the >8-stream processing-heavy exception, the multi-sink clock-stall exception, and the audio-classification omission.
...
| `waylandsink` | Wayland display sink. | `display`, `drm-device`, `force-aspect-ratio`, `fullscreen` (bool, default false), `fullscreen-output`, `rotate-method`, `sync` (bool, default true): sync to pipeline clock — keep `sync=true` for every source type including live camera sources; use `sync=false` only when more than 8 independent concurrently active input streams are combined with a processing-heavy topology (shared/batched inference, or a composer grid where HTP/batch preroll makes frames arrive well behind their PTS), when display runs alongside an encode/file/metadata sink that can stall the display clock (stream-count-independent), or when the user explicitly requests lower latency over A/V sync, `async` (bool, default true): enable async state changes — use `async=false` only for snapshot/still sinks, `enable-last-sample` (bool, default true): maintain last-sample buffer — set `enable-last-sample=false` in multi-sink camera pipelines to reduce memory usage | Sink (`sink`, always): `video/x-raw` formats `{ BGR10A2_LE, RGB10A2_LE, AYUV, RGBA, ARGB, BGRA, ABGR, BGR10x2_LE, RGB10x2_LE, P010_10LE, NV12_10LE40, Y444, v308, RGBx, xRGB, BGRx, xBGR, RGB, BGR, Y42B, NV16, NV61, YUY2, YVYU, UYVY, I420, YV12, NV12, NV21, Y41B, YUV9, YVU9, BGR16, RGB16, NV12_Q08C }`, `video/x-raw(memory:DMABuf)` format `DMA_DRM`. No src pad. **Skill default: `fullscreen=true sync=true` for every source type, including live camera sources** — override with `sync=false fullscreen=true enable-last-sample=false` only for the multi-sink clock-stall case, or `sync=false fullscreen=true` for the >8-stream processing-heavy multistream/composer-grid case or an explicit user low-latency request. |
...
- `fpsdisplaysink` — wraps a real video sink and overlays live frame-rate stats; key props: `video-sink` (object of type `GstElement` — a real sink element instance such as a `waylandsink`, not a factory-name string; assign the actual element object, e.g. via `g_object_set(fps, "video-sink", wayland_sink, NULL)`), `text-overlay` (bool, default true — draws FPS as text on the video), `signal-fps-measurements` (bool, default false), `sync` (bool, default true — forwarded to the wrapped sink if it supports it), `fps-update-interval` (int ms, default 500). Use when the user asks for an FPS counter/overlay; otherwise use the wrapped sink (e.g. `waylandsink`) directly.
...
> **Device-verified `video/x-raw` format — use `RGBA`, never `BGRA`.** On the target QLI build, `gst-inspect-1.0 qtimlpostprocess` reports the src-pad video caps as `video/x-raw, format={ RGBA, RGBx }` — **`BGRA` is NOT in the supported set.** A `BGRA` render capsfilter after `qtimlpostprocess` fails to link (`could not link ... can't handle caps video/x-raw, format=(string)BGRA`), or stalls the composer in preroll. Every render-overlay caps fed from `qtimlpostprocess` must use `RGBA` (`qti::VideoFilter().format("RGBA")` in C++). Older docs/examples that say "BGRA" are stale for this plugin build.
>
> **Do NOT pin an explicit width/height on the render caps right after `qtimlpostprocess` when a downstream `qtivcomposer` sizes the tile** — pinning makes caps fixation fail. Device-verified error: `Fixated width in filter caps is not supported with current post-process type!` — this occurs regardless of whether `format=` is `RGBA` or `BGRA`, so relabeling the format alone does not fix it. Leave the size unset and let the composer sink-pad `dimensions` scale it. Pinning is only appropriate when nothing downstream sizes the branch.
...
**Typical/example dimensions** for modules that negotiate to `video/x-raw` (illustrative, not fixed — dimensions come from the model's own tensor shape; the format is always `RGBA` per the device-verified note above, and should be left unpinned when a composer sizes the tile):

| Module | Example `video/x-raw` output |
|---|---|
| `yolov8`, `yolov5`, `yolo-nas`, `qfd`, `hrnet`, `qpd` | `RGBA` (native size; do not pin when composed) |
| `mobilenet`, `mobilenet-softmax`, `qfr`, `qfr-softmax` | `RGBA` (size derived from font size × label count) |
| `deeplab-argmax` | `RGBA` (native mask size; do not pin when composed) |
| `midas-v2` | `RGBA` (native depth-map size; do not pin when composed) |
| `srnet` | `RGBA` (upscaled size; device-verified — do not pin when composed) |
| `yamnet` | `RGBA` (label-panel size) |
