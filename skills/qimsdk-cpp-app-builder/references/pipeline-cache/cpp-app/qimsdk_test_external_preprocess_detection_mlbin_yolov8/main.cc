...
//
// Writes in place into `tensor.data`, which points directly at the bin's
// internal preprocess output GstMLFrame block. The SDK re-validates the
// tensor's data pointer and size after the callback returns, so the block must
// be filled in place - repointing tensor.data at another buffer fails.
//
...
bool convert_nv12_to_nhwc_i8(const MLVideoImage& image,
  ...
  if (blit) {
    ...
    dst_w = std::max(
        1, std::min<int>(blit->destination.w, static_cast<int>(out_w) - dst_x));
    dst_h = std::max(
        1, std::min<int>(blit->destination.h, static_cast<int>(out_h) - dst_y));
  }
...
//            → qtimlvideotflitebin(external preprocess callback)
//            → qtivoverlay → waylandsink
//
//  The pipeline reads an MP4/H.264 file, decodes it through the hardware decoder,
//  runs YOLOv8 object detection with external preprocessing callback logic in mlbin,
...
void create_and_execute_pipeline() {
  ...
  // Runs preprocessing, inference, and postprocessing inside mlbin.
  //
  // preprocess-engine=none disables the bin's internal preprocessing path,
  // handing the conversion to the callback registered below. Unlike the Python
  // binding, the C++ set_preprocess_handler() does not set this implicitly.
  ...
  mlbin.set_preprocess_handler(
      [](const MLVideoBlits& blits, MLFrame& output) {
        ...
        // single-frame batching that the bin's preprocess uses by default.
        ...
        if (!ok) {
          std::cout << "preprocess failed: conversion routine failed"
                    << std::endl;
        }
        return ok;
      });
  ...
  Pipeline pipeline("ml-external-preprocess-detection-mlbin");
  pipeline.add(src)
          .add(demux)
          .add(parse)
          .add(decoder)
