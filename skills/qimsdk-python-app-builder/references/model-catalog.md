...
| Model | Float32 filename | W8A8 filename | Module | Labels file | Settings | Delegate (float) | Delegate (w8a8) | Notes |
|---|---|---|---|---|---|---|---|---|
| YAMNET | yamnet_float.tflite | — | yamnet | yamnet.json | json | none | — | audio pipeline; uses qtimlaconverter (sample-rate=16000) instead of qtimlvconverter |
...
**Topology:** use the Python AI/daisy-chain references for the full 3-stage pipeline shape. Do not duplicate topology rules here — this table resolves only the per-model module/delegate facts.
