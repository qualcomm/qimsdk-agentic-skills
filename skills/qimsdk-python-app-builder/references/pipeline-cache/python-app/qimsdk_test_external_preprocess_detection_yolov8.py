...
"""Custom object-detection preprocess example (native YOLOv8 postprocess)."""
...
#      split -> mlmuxer
#      split -> q1 -> preprocessing(custom callback) -> q2 -> inferencing -> q4
...
#  runs YOLOv8 object detection with an external Python preprocessing callback
#  feeding the native postprocessing element, overlays the detected objects,
...
def create_and_execute_pipeline() -> None:
    ...
    pipeline = (
        Pipeline("ml-external-preprocess-detection")
        .add(src)
        .add(demux)
        .add(parse)
        .add(decoder)
        ...
        .link("split", "mlmuxer")
        .link("split", "q1", "preprocessing", "q2", "inferencing", "q4", "postprocessing",
              "mlf", "mlmuxer", "q5", "overlay", "display")
    )
