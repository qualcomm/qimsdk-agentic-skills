...
"""BM25 retrieval over the qimsdk-python-app-builder pipeline example indexes.
...
the two example corpora shipped alongside this skill:
    ...
    skills/qimsdk-python-app-builder/references/pipeline-index.gst-launch.jsonl  (gst-launch/*.sh one-liners -- topology reference only, never leveraged verbatim into Python output)
    skills/qimsdk-python-app-builder/references/pipeline-index.python-app.jsonl  (python-app/<slug>.py real qimsdk SDK Python apps -- the leverageable pool)

Each line in those files is a JSON object: {"prompt": "<nl description>",
"path": "gst-launch/<slug>.sh"} or {"prompt": "...", "path": "python-app/<slug>.py"}.
...
The two .jsonl files are tokenized once and cached to
    skills/qimsdk-python-app-builder/references/pipeline-cache/.index-cache.json
...
CLI
    python rank_examples.py --query "<text>" [--pool {gst-launch,python-app,both}]
                             [--top-k N] [--task TAG]
...
EXAMPLES
    ...
    python rank_examples.py --query "gesture recognition" --pool python-app
    ...
    python rank_examples.py --query "detect objects from usb-camera" --task object-detection --pool python-app
...
INDEX_FILES = {
    ...
    "python-app": CORPUS_DIR / "pipeline-index.python-app.jsonl",
}
POOL_NAMES = ("gst-launch", "python-app")  # fixed, deterministic iteration order
...
SYNONYMS = {
    ...
    "hrnet": ["pose"],
    "keypoint": ["pose"],
    # multi-model chaining -- corpus is split between "daisychain" (python-app)
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="BM25 retrieval over the qimsdk-python-app-builder pipeline example indexes.",
    ...
    parser.add_argument(
        "--pool",
        choices=["gst-launch", "python-app", "both"],
        default="both",
